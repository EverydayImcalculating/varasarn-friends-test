...
const READ_FUNCTIONS = [
  ...
  'list_my_offering_proposals', 'list_my_timetable', 'list_pending_offering_proposals',
...
const WRITE_FUNCTIONS = [
  'add_my_timetable_offering', 'archive_course', 'bulk_import_offerings', 'clear_my_timetable',
  'create_academic_period', 'create_category', 'create_course', 'create_offering', 'create_offering_proposal',
  'create_review', 'grant_administrator', 'import_legacy_reviews', 'merge_course', 'moderate_review',
  'remove_my_timetable_offering', 'replace_my_timetable_offering', 'resolve_offering_proposal',
...
describe('api write classification', () => {
  ...
  it('classifies exactly the 42 known functions with no overlap or gap', () => {
    expect(READ_FUNCTIONS.length + WRITE_FUNCTIONS.length).toBe(42)
    expect(new Set([...READ_FUNCTIONS, ...WRITE_FUNCTIONS]).size).toBe(42)
  })
})
