...
const READ_FUNCTIONS = [
  ...
  'list_my_offering_proposals', 'list_my_timetable', 'list_my_reported_timetable', 'list_pending_offering_proposals',
...
describe('api write classification', () => {
  ...
  it('classifies exactly the 45 known functions with no overlap or gap', () => {
    expect(READ_FUNCTIONS.length + WRITE_FUNCTIONS.length).toBe(45)
    expect(new Set([...READ_FUNCTIONS, ...WRITE_FUNCTIONS]).size).toBe(45)
  })
})
