...
### 2026-09-24 — Owner hid the proposal-box UI again; auto-approve backend left in place

The owner committed directly (`07facca`, no Claude Code session) re-adding `v-if="false"` to `.proposal-box` in [`src/App.vue`](../../src/App.vue), undoing this feature's UI visibility. Confirmed with the owner: intentional, the box shouldn't be shown. The commit skipped this repo's usual pre-commit checks and broke `tests/review-to-timetable-app.test.ts`'s `'adds a new class section instantly, without admin approval'` test, since `v-if="false"` removes the element from the DOM entirely (`wrapper.get('.proposal-box')` couldn't find it) — no spec entry was logged for it either.

Removed that test (same precedent as hiding the `รายวิชาที่เลือก` list earlier this session: a `v-if="false"` UI path gets its exercising test trimmed, not left to bit-rot). Left everything else about this feature as-is: migration `0034_auto_approve_offering_proposals.sql` stays live in production (any proposal submitted directly against the RPC would still auto-approve), `list_my_offering_proposals`/`create_offering_proposal` mock plumbing stays in the test fixture (still exercised — `myProposals` still loads on every review open, just isn't rendered), and the copy/logic changes from the original implementation are untouched. Only the box's visibility and the one test that asserted on it changed.

`npm test -- --run` (83/83, one test removed from 84) and `npm run build` passed.
