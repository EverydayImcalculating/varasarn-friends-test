...
### 2026-09-23 — Migration 0025 applied to production; update_offering confirmed live and admin-gated

With owner approval, ran `npm run db:migrate` against the production branch and `neon data-api refresh-schema --project-id soft-surf-84712820 --branch production`. A read-only check confirms `api.update_offering(uuid,integer,text,text,text,integer,time,time)` now exists in the production `api` schema and the applied-migrations log advanced from 25 to 26 rows. An unauthenticated `rpc/update_offering` request against the live Data API was rejected with `HTTP 400` and `missing authentication credentials`, matching every other protected RPC's behavior. Rebuilt commit `f674c72` locally (`npm run build`) and confirmed the deployed test app (`https://varasarn-friends-test-tau.vercel.app/`) serves the identical asset hashes, so the dashboard edit path described above is now live end to end at the database and deployment layer.

`npm test -- --run` passed 24 tests on this commit.

**Still open:** live acceptance that a signed-in non-administrator is denied by `require_administrator()` when calling `update_offering` directly, and browser confirmation that an administrator's correction appears in `offering_audit` and in the dashboard, both need a real signed-in Google session. No Google test-account credentials are available to this session (see Ticket 01's blocker comment for the same limitation). Status stays `needs-info` pending that evidence.
