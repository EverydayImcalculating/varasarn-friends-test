...
## Follow-up from deployed screenshots (2026-09-26)

The owner observed a visible `>` after the stars in the course dialog, and no rating lines on catalog cards at `varasarn-friends-test-tau.vercel.app`. Fix these within the decisions above; do not redesign the rating display.

1. **Remove the stray glyph.** In `src/App.vue`, the self-closing `StarRating` in the course-review card is followed by a leftover `>` before the term span. The My Reviews card has the same leftover `>` before its publication badge. Remove those literal characters and inspect the other rating placements for the same mistake. Keep the five star glyphs and hidden Thai labels. Extend the existing App-mounted course-dialog and My Reviews tests with literal assertions that no `>` appears between each rating and the following term or badge.
2. **Trace the missing catalog fields in the deployed test environment.** The card intentionally renders no summary when `review_count` is absent; that explains the screenshot's empty rating area, but the screenshot cannot establish why the field is absent. Confirm the deployed frontend revision and inspect a real `list_approved_catalog` response from the test site. On the exact Neon branch used by that deployment, read the function definition with `pg_get_functiondef` and check `has_function_privilege('authenticated', 'api.list_approved_catalog()', 'EXECUTE')`. Check whether the response includes `review_count` and `average_rating` for AP164 and a course with no reviews. If migration 0036 is missing, apply it only after confirming this is a non-production branch; if the branch is production, stop for the owner's explicit approval. If the fields are present, diagnose the frontend mapping/deployment instead. Preserve the optional-field deploy-order guard and the existing paginated catalog fetch.
3. **Accept only the intended rendered result.** After the frontend and the matching database function are in place, AP164 should show a rating line matching the reviews in its unfiltered course dialog. If the two pictured 4- and 5-star reviews are the only qualifying reviews, the RPC should return `review_count: 2` and `average_rating: 4.5`, and the card should show `★★★★★ 4.5 · 2 รีวิว`. Courses with no qualifying reviews should show `ยังไม่มีรีวิว`; rows from an old RPC should still show no line. Recheck the catalog, course dialog, My Reviews, and admin reviews at 393×852, 360×780, and one desktop width. Run `npm test -- --run` and `npm run build` before reporting.
...
### 2026-09-26: Deployed screenshot follow-up

The owner's screenshots showed the course-dialog `>` template regression and catalog cards without rating lines. Removed the stray characters from both the course-dialog and My Reviews templates, and added App-mounted assertions that their rendered review cards contain no `>`. `npm test -- --run` passes (16 files, 97 tests); `npm run build` passes (with the existing large-chunk warning). A read-only check of the Neon branch named `production` confirmed `api.list_approved_catalog()` still returns only `id`, `code`, `name_th`, and `category_name`, which explains why the optional rating line is absent. No migration was applied. The exact database branch used by the test deployment has not been established; apply migration 0036 only on a confirmed non-production branch, then verify the deployed catalog response and requested viewports.

### 2026-09-26: Production migration and Data API refresh

The owner explicitly authorized applying migration 0036 to production, overriding the earlier production restriction. Neon's named snapshot limit was reached, so a recovery branch was created from production first: `recovery-before-0036-20260925` (`br-raspy-sun-b3wouku2`, parent LSN `0/2307A30`). Drizzle then applied migration 0036 to production. The catalog RPC now returns `review_count` and `average_rating`; `authenticated` retains EXECUTE. AP164 returned 2 qualifying reviews with an average rating of 4.5. Neon's Data API schema cache was refreshed for database `db` on the production branch. Live signed-in browser acceptance and mobile viewport checks remain pending.
