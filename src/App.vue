  ...
  <main>
    ...
    <section v-else class="container pb-5">
      ...
      <section v-else-if="timetable">
        ...
        <template v-else><div class="timetable-container"><div class="timetable-grid"><div class="time-header-row"><div v-for="hour in 12" :key="hour" class="time-header-slot">{{ String(hour + 7).padStart(2, '0') }}:00</div></div><div v-for="day in [1,2,3,4,5,6,7]" :key="day" class="day-row"><div class="day-label">{{ dayNames[day] }}</div><div class="day-track"><div v-for="entry in timetableEntries.filter((item) => item.day_of_week === day)" :key="`${entry.offering_id ?? entry.review_id}-${day}`" class="timetable-course" :class="courseColor(entry.course_code)" :style="timetableStyle(entry)"><strong>{{ entry.course_code }} ({{ entry.section }})</strong><span v-if="entry.instructor_name" class="text-truncate">{{ entry.instructor_name }}</span><span>{{ timeValue(entry.starts_at) }}–{{ timeValue(entry.ends_at) }}</span></div></div></div></div></div><div class="review-box"><h2 class="h5">รายวิชาที่เลือก</h2><div v-for="entry in timetableEntries.filter((item, index, items) => items.findIndex((other) => (other.offering_id ?? other.review_id) === (item.offering_id ?? item.review_id)) === index)" :key="entry.offering_id ?? entry.review_id!" class="d-flex justify-content-between align-items-center border-bottom py-2"><span><strong>{{ entry.course_code }}</strong> {{ entry.course_name }} · กลุ่ม {{ entry.section }}<small v-if="entry.source === 'reported'" class="text-muted"> · ข้อมูลจากรีวิว</small></span><button class="btn btn-sm btn-outline-danger" @click="removeFromTimetable(entry)">ลบออก</button></div></div></template>
      </section>
