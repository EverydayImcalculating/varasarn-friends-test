...
async function addCourse() { try { await saveCourse(); await Promise.all([openDashboard(), loadCatalog()]) } catch (cause) { error.value = cause instanceof Error ? cause.message : 'ไม่สามารถบันทึกรายวิชาได้' } }
...
async function submitCourseModal() { try { await saveCourse(); courseModalOpen.value = false; await loadCatalog() } catch (cause) { error.value = cause instanceof Error ? cause.message : 'ไม่สามารถบันทึกรายวิชาได้' } }
  ...
  <main>
    ...
    <section v-else class="container pb-5">
      ...
      <section v-else-if="timetable">
        <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3"><div><h1 class="h3 text-purple"><i class="bi bi-grid-3x3-gap-fill me-2"></i>ตารางเรียนส่วนตัว</h1><p class="text-muted mb-0">ตารางนี้ผูกกับบัญชีของคุณ</p></div><div class="d-flex gap-2"><button class="btn btn-outline-danger shadow-sm" @click="clearTimetable"><i class="bi bi-trash-fill me-1"></i>ล้างตาราง</button><button class="btn btn-purple shadow-sm" @click="timetable = false"><i class="bi bi-arrow-left-circle-fill me-1"></i>หน้าหลัก</button></div></div>
        ...
        <template v-else><div class="timetable-container"><div class="timetable-grid"><div class="time-header-row"><div v-for="hour in 12" :key="hour" class="time-header-slot">{{ String(hour + 7).padStart(2, '0') }}:00</div></div><div v-for="day in [1,2,3,4,5,6,7]" :key="day" class="day-row"><div class="day-label">{{ dayNames[day] }}</div><div class="day-track"><div v-for="entry in timetableEntries.filter((item) => item.day_of_week === day)" :key="`${entry.offering_id ?? entry.review_id}-${day}`" class="timetable-course" :class="courseColor(entry.course_code)" :style="timetableStyle(entry)" title="คลิกเพื่อลบวิชานี้" @click="confirmRemoveFromTimetable(entry)"><strong>{{ entry.course_code }} ({{ entry.section }})</strong><span v-if="entry.instructor_name" class="text-truncate">{{ entry.instructor_name }}</span><span>{{ timeValue(entry.starts_at) }}–{{ timeValue(entry.ends_at) }}</span></div></div></div></div></div><div class="review-box"><h2 class="h5">รายวิชาที่เลือก</h2><div v-for="entry in timetableEntries.filter((item, index, items) => items.findIndex((other) => (other.offering_id ?? other.review_id) === (item.offering_id ?? item.review_id)) === index)" :key="entry.offering_id ?? entry.review_id!" class="d-flex justify-content-between align-items-center border-bottom py-2"><span><strong>{{ entry.course_code }}</strong> {{ entry.course_name }} · กลุ่ม {{ entry.section }}<small v-if="entry.source === 'reported'" class="text-muted"> · ข้อมูลจากรีวิว</small></span><button class="btn btn-sm btn-outline-danger" @click="removeFromTimetable(entry)">ลบออก</button></div></div></template>
      </section>
