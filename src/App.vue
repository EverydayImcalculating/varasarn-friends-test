...
import { AdminService, type AcademicPeriod, type Category, type ManagedCourse, type MergePreview, type ModerationReview, type PendingProposal, type RoleAssignment, type VerifiedAccount } from './services/admin'
...
async function openCourse(course: Course) {
  ...
  await Promise.all([offerings.value[0] ? loadReviews(offerings.value[0].id) : Promise.resolve(), loadMyProposals()])
}
async function loadReviews(offeringId = offerings.value[0]?.id) {
  if (!offeringId || !service.value) return
  reviews.value = await service.value.listVisible(offeringId, { rating: reviewRatingFilter.value || undefined, semester: reviewSemesterFilter.value || undefined, academicYear: reviewYearFilter.value || undefined })
}
...
async function moderateReview(id: string, state: ModerationReview['moderation_state']) { if (!adminService.value) return; try { await adminService.value.moderateReview(id, state, moderationReasons.value[id] ?? ''); moderationReasons.value[id] = ''; await loadModerationReviews() } catch (cause) { error.value = cause instanceof Error ? cause.message : 'ไม่สามารถเปลี่ยนสถานะรีวิวได้' } }
